...
/**
 * @class AuthJwtGuard
 * @description registered globally via APP_GUARD in AuthModule.
 *
 ...
 */
@Injectable()
class AuthJwtGuard extends AuthGuard('jwt') implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const request: Typed.Auth.AuthenticationRequest = context.switchToHttp().getRequest()
    const user: Typed.Auth.Profile = request.user
    ...
    const requiredRoles = this.reflector.getAllAndOverride<Typed.User.Role[]>(ROLES_KEY, [handler, cls])
    ...
    const requiredAbilities = this.reflector.getAllAndOverride<Typed.User.Ability[]>(ABILITIES_KEY, [handler, cls])
    if (requiredAbilities?.length) {
      const roleAbilities: Typed.User.Ability[] = Constants.ROLE_ABILITIES[user.role] ?? []
      ...
      const userAbilities: Typed.User.Ability[] = user.abilities ?? []
